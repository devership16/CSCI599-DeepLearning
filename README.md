...
**GCNN Encoder**
The input to the encoder is the nodal feature vectors $h_i$, and the graph $G = (V, R)$ with nodes $v_i~ \epsilon ~V$ and labeled edges $(v_i, r, v_j)$ where is an edge type.  The output is a d-dimensional embedding $h_{i}^{k+1}$ for each node. 
